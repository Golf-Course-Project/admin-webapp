...
class ListBlogs extends React.Component<IProps, {}> {
  ...
  private handleCreateNewBlog = () => {
    this.setState({ isCreating: true, errorMsg: '' });

    const client: BlogService = new BlogService();
    
    client.create().then(async (response: IFetchBlogApiResponse) => {
      if (response.messageCode !== 200 || !response.success) {
        this.setState({ 
          errorMsg: response.message || 'Failed to create blog', 
          isCreating: false 
        });
        return;
      }
      ...
      this.setState({ errorMsg: error.message, isCreating: false });
    });
  };
  ...
  private handleBlogUpdate = (blog: IBlog | null) => {
    ...
    const newData = this.state.data.map(item => 
      item.id === blog.id ? { ...item, title: blog.title } : item
    );
    ...
    return (
      <Box>   
              ...
              >
                <Box>
                  ...
                  >
                    ...
                    >
                      {this.state.isCreating ? 'Creating...' : 'Create new post'}
                    </Button>
                  </Box>
                </Box>
              </Grid>
        ...
        <Box sx={this.state.action === 'normal' ? { display: 'block' } : { display: 'none' }}>
          <Box marginBottom={2} display="flex" justifyContent="space-between" alignItems="center">
            ...
            <Button
              ...
              disabled={this.state.isCreating}
            >
              {this.state.isCreating ? 'Creating...' : 'Create New'}
            </Button>
          </Box>
        ...
        <EditBlog
          ...
          onBlogUpdate={(e: any) => this.handleBlogUpdate(e)}                            
        >
        </EditBlog>   
      </Box>
    );
